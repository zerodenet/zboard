<template>
  <div v-if="open && selected" class="announcement-backdrop" @click.self="closePrompt">
    <section :class="['announcement-dialog', { 'reader-mode': readerMode }]" role="dialog" aria-modal="true" aria-labelledby="announcement-dialog-title">
      <header class="dialog-header">
        <div class="dialog-context"><span class="eyebrow">NOTICE DESK</span><span>{{ visible.length }} 条待阅读</span></div>
        <button type="button" class="close-button" aria-label="暂时关闭公告" @click="closePrompt">×</button>
      </header>
      <nav v-if="priorityItems.length && otherItems.length" class="announcement-tabs" role="tablist" aria-label="公告分类">
      ...
      <div class="announcement-layout">
        ...
        <aside v-if="readerMode" class="announcement-queue" aria-label="本分类公告">
          <span class="queue-title">本分类 {{ candidates.length }} 条</span>
          <button v-for="(item, index) in candidates" :key="keyOf(item)" type="button" :aria-pressed="keyOf(item) === keyOf(selected)" @click="selectItem(item)">
...
watch(() => priorityItems.value.map(keyOf).join(','), value => {
  ...
  // Keep one layout for the entire open session, even as notices are read.
  if (!open.value) readerMode.value = visible.value.length > 1
  tab.value = 'priority'
...
.announcement-dialog { width: min(660px, 100%); max-height: min(760px, calc(100vh - 40px)); overflow: auto; border: 1px solid var(--line-strong); border-radius: 22px; background: var(--surface); box-shadow: 0 32px 100px var(--sidebar-shadow); }
.announcement-dialog.reader-mode { display: flex; flex-direction: column; width: min(920px, 100%); height: min(640px, calc(100vh - 40px)); height: min(640px, calc(100dvh - 40px)); overflow: hidden; }
.reader-mode .dialog-header, .reader-mode .announcement-tabs { flex-shrink: 0; }
.reader-mode .announcement-layout { flex: 1; min-height: 0; grid-template-columns: minmax(0, 1fr) 250px; overflow: hidden; }
.reader-mode .announcement-feature { display: flex; min-height: 0; flex-direction: column; overflow: hidden; }
.reader-mode .announcement-reading { flex: 1; min-height: 0; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; }
.reader-mode .announcement-feature footer { flex-shrink: 0; }
.reader-mode .announcement-queue { min-height: 0; overflow-y: auto; overscroll-behavior: contain; }
.dialog-header { display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 16px 20px; border-bottom: 1px solid var(--line); }.dialog-context { display: flex; align-items: center; gap: 10px; color: var(--muted); font-size: 10px; }.eyebrow { color: var(--primary); font-size: 9px; font-weight: 850; letter-spacing: .16em; }.close-button { display: grid; width: 32px; height: 32px; place-items: center; border: 1px solid var(--line); border-radius: 50%; background: transparent; color: var(--muted); cursor: pointer; font-size: 20px; line-height: 1; }
.announcement-tabs { display: flex; gap: 6px; padding: 0 20px; border-bottom: 1px solid var(--line); }.announcement-tabs button { position: relative; padding: 10px 11px 11px; border: 0; background: transparent; color: var(--muted); cursor: pointer; font: inherit; font-size: 11px; font-weight: 750; }.announcement-tabs button.active { color: var(--text); }.announcement-tabs button.active::after { position: absolute; right: 8px; bottom: -1px; left: 8px; height: 2px; background: var(--primary); content: ''; }.announcement-tabs button span { margin-left: 5px; color: var(--primary); }.announcement-tabs button:disabled { cursor: default; opacity: .45; }
.announcement-layout { display: grid; grid-template-columns: minmax(0, 1fr); }.announcement-feature { padding: 22px 24px 24px; border-left: 4px solid var(--primary); }.announcement-feature.severity-critical { border-color: var(--danger); }.announcement-feature.severity-warning { border-color: var(--warning); }.announcement-feature.severity-success { border-color: var(--success); }.feature-meta { display: flex; justify-content: space-between; gap: 12px; color: var(--muted); font-size: 10px; }.feature-meta span { color: var(--primary); font-weight: 800; }.announcement-feature h2 { margin: 12px 0 16px; font-size: clamp(24px, 4vw, 30px); line-height: 1.2; }.announcement-feature footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--line); color: var(--muted); font-size: 10px; }.announcement-feature footer a { color: var(--primary); font-weight: 750; text-decoration: none; }.acknowledge { padding: 9px 13px; border: 0; border-radius: 9px; background: var(--primary); color: var(--text-inverse); cursor: pointer; font: inherit; font-size: 11px; font-weight: 800; }
.announcement-queue { display: grid; align-content: start; gap: 6px; padding: 20px 18px; border-left: 1px solid var(--line); background: var(--surface-soft); }.queue-title { padding: 0 8px 8px; color: var(--muted); font-size: 9px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; }.announcement-queue button { display: grid; grid-template-columns: 28px minmax(0, 1fr); gap: 8px; padding: 11px 8px; border: 0; border-radius: 10px; background: transparent; color: inherit; cursor: pointer; text-align: left; }.announcement-queue button:hover { background: var(--surface); }.queue-rank { color: var(--line-strong); font-size: 16px; font-weight: 900; }.announcement-queue button span:last-child { display: grid; gap: 4px; min-width: 0; }.announcement-queue strong { white-space:normal; overflow-wrap:anywhere; font-size:11px; }.announcement-queue small { color: var(--muted); font-size: 9px; }
.announcement-feature { min-width: 0; }
.announcement-queue button[aria-pressed="true"] { background: var(--surface); color: var(--primary); box-shadow: inset 3px 0 var(--primary); }
.announcement-queue button[aria-pressed="true"] .queue-rank { color: var(--primary); }
@media (max-width: 720px) {
  .announcement-dialog.reader-mode { height: min(760px, calc(100vh - 40px)); height: min(760px, calc(100dvh - 40px)); }
  .reader-mode .announcement-layout { grid-template-columns: minmax(0, 1fr); grid-template-rows: minmax(0, 1fr) 92px; }
  .reader-mode .announcement-queue { display: flex; align-items: stretch; gap: 8px; padding: 10px 16px; border-top: 1px solid var(--line); border-left: 0; overflow-x: auto; overflow-y: hidden; }
  .announcement-queue .queue-title { display: none; }
  .announcement-queue button { flex: 0 0 min(230px, 72vw); align-content: center; }
  .announcement-feature { padding: 20px 18px 22px; }
  .announcement-feature footer { align-items: flex-start; flex-direction: column; }
  .acknowledge { width: 100%; }
  .dialog-header, .announcement-tabs { padding-inline: 18px; }
}
</style>
