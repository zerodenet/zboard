...
const related = computed(() => {
  ...
  return candidates.value.filter(item => keyOf(item) !== currentKey).slice(0, 4)
})
...
.announcement-dialog.has-queue { width: min(920px, 100%); }
...
@media (max-width: 720px) { .announcement-layout, .announcement-layout.has-queue { grid-template-columns: 1fr; }.announcement-queue { border-top: 1px solid var(--line); border-left: 0; }.announcement-feature { padding: 20px 18px 22px; }.announcement-feature footer { align-items: flex-start; flex-direction: column; }.acknowledge { width: 100%; }.dialog-header, .announcement-tabs { padding-inline: 18px; } }
</style>
