...
const mocks=vi.hoisted(()=>({list:vi.fn(),save:vi.fn(),remove:vi.fn(),detail:vi.fn(),runtime:vi.fn()}))
vi.mock('../api/client',()=>({fetchNodeProxyPools:mocks.list,saveNodeProxyPool:mocks.save,deleteNodeProxyPool:mocks.remove,fetchNodeProxyPoolConfig:mocks.detail,fetchNodeProxyPoolRuntime:mocks.runtime}))
...
describe('node shared pools',()=>{
 beforeEach(()=>{vi.clearAllMocks();mocks.list.mockResolvedValue([{id:3,node_id:7,name:'现有池',revision:4,entry_count:2}]);mocks.save.mockResolvedValue({});mocks.detail.mockResolvedValue({pool:{id:3,node_id:7,name:'现有池',revision:4},config:poolFixture(),compiled:{}})})
