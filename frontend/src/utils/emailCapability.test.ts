...
describe('SMTP and email template capability', () => {
  ...
  it('queues registration delivery through the persisted task boundary', () => {
    const capability = read('backend/internal/capabilities/messaging/welcome.go')
    const repository = read('backend/internal/adapters/persistence/messagingstore/welcome.go')
    const migration = read('backend/migrations/0001_init.up.sql')
    expect(capability).toContain('EnqueueRegisteredAccount(context.Context, uint)')
    expect(capability).toContain('func (s RegistrationWelcome) Enqueue(')
    expect(repository).toContain('IdempotencyKey: key')
    expect(repository).toContain('notification.enqueue')
  ...
  it('gates registration with a short-lived email code and visualizes persisted task status', () => {
    ...
    const registration = read('backend/internal/capabilities/identity/registration.go')
    const tasks = read('frontend/src/views/Tasks.vue')
    const client = read('frontend/src/api/client.ts')
