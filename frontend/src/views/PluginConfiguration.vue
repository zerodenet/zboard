<template>
  <div class="plugins-page plugin-configuration-page">
    ...
    <template v-if="plugin">
      ...
      <PageAlert v-if="!configurable" tone="warning">{{ plugin.state === 'uninstalled' ? '插件已卸载，重新安装后可继续配置。' : !plugin.compatibility.compatible ? '此插件与当前宿主不兼容，暂时无法配置。' : '此插件未声明配置能力。' }}</PageAlert>
      <template v-else>
...
import { usePluginDetail, pluginDetailPath, hasPluginConfig } from '../plugins/usePluginManagement'
...
const configPage = computed(() => plugin.value?.manifest.contributions.pages.find(p => p.surface === 'admin' && p.purpose === 'configuration'))
let generation = 0
