<template>
  <section class="standard-page">
      ...
      <main v-else-if="selectedOrder" class="business-detail stack">
        ...
        <section class="payment-events stack" aria-label="支付事件时间线">
          ...
          <DataTable v-if="paymentEvents.length" caption="订单支付事件安全摘要" :row-count="paymentEventTotal" :min-width="760">
            ...
            <tbody><tr v-for="event in paymentEvents" :key="event.id"><td class="table-primary-column"><div class="cell-title"><strong>{{ eventTypeName(event.event_type) }}</strong><TableText :value="safeEventReference(event.provider_event_id)" /></div></td><td><TableText :value="providerName(event.provider)" /></td><td class="value-cell" data-column-priority="1">{{ formatCurrency(event.amount_minor, selectedOrder.currency || 'CNY') }}</td><td><StatusBadge :tone="event.signature_valid ? 'success' : 'danger'">{{ event.signature_valid ? '验签通过' : '验签失败' }}</StatusBadge></td><td data-column-priority="2"><TimeBadge :value="event.processed_at" /></td><td data-column-priority="2"><TimeBadge :value="event.created_at" /></td></tr></tbody>
          </DataTable>
    ...
    <OrderSubscriptionDialog :order-id="accessOrderID" @close="accessOrderID = 0" />
