<template>
  <section class="standard-page">
    ...
    <DataWorkbench :total="recordTotal" :loading="recordLoading" :refreshing="recordRefreshing">
      ...
      <template #actions><span class="workbench-note">流量按实际大小自动显示 B / KB / MB / GB</span></template>

      <DataTable v-if="records.length" caption="流量上报记录列表" :row-count="recordTotal" :min-width="980" table-class="traffic-table">
          <thead><tr><th class="table-primary-column">上报时间</th><th class="numeric-column" data-column-priority="3">用户 ID</th><th class="numeric-column">订阅 ID</th><th class="numeric-column" data-column-priority="2">节点 ID</th><th class="numeric-column" data-column-priority="2">端点 ID</th><th class="numeric-column" data-column-priority="3">原始流量</th><th class="numeric-column" data-column-priority="3">倍率 (×)</th><th class="numeric-column">计费流量</th></tr></thead>
          <tbody><tr v-for="record in records" :key="record.id"><td class="table-primary-column"><TimeBadge :value="record.record_at" /></td><td class="numeric-column" data-column-priority="3">{{ record.user_id || '—' }}</td><td class="numeric-column">{{ record.subscription_id || '—' }}</td><td class="numeric-column" data-column-priority="2">{{ record.node_id || '—' }}</td><td class="numeric-column" data-column-priority="2">{{ record.protocol_endpoint_id || '—' }}</td><td class="numeric-column" data-column-priority="3">{{ formatBytes(record.raw_bytes) }}</td><td class="numeric-column" data-column-priority="3">{{ formatMultiplier(record.protocol_multiplier_milli) }}</td><td class="numeric-column"><strong>{{ formatBytes(record.used_bytes) }}</strong></td></tr></tbody>
      </DataTable>
    ...
    <DataWorkbench class="reconciliation-workbench" :total="reconciliationTotal" :loading="reconciliationLoading" :refreshing="reconciliationRefreshing">
      ...
      <DataTable v-if="reconciliation.length" caption="订阅流量对账结果" :row-count="reconciliationTotal" :min-width="1120" table-class="reconciliation-table">
          <thead><tr><th class="numeric-column table-primary-column">订阅 ID</th><th class="numeric-column" data-column-priority="3">用户 ID</th><th class="numeric-column" data-column-priority="3">套餐 ID</th><th>订阅状态</th><th class="numeric-column" data-column-priority="2">订阅累计</th><th class="numeric-column" data-column-priority="2">记录汇总</th><th class="numeric-column">差异</th><th>对账结果</th><th class="table-action-column"><span class="sr-only">操作</span></th></tr></thead>
          <tbody><tr v-for="item in reconciliation" :key="item.subscription_id"><td class="numeric-column table-primary-column">{{ item.subscription_id }}</td><td class="numeric-column" data-column-priority="3">{{ item.user_id }}</td><td class="numeric-column" data-column-priority="3">{{ item.plan_id }}</td><td><StatusBadge :tone="item.status === 'active' ? 'success' : 'neutral'">{{ subscriptionStatusName(item.status) }}</StatusBadge></td><td class="numeric-column" data-column-priority="2">{{ formatBytes(item.flow_used) }}</td><td class="numeric-column" data-column-priority="2">{{ formatBytes(item.recorded_bytes) }}</td><td class="numeric-column" :class="{ 'danger-text': item.difference }">{{ formatSignedBytes(item.difference) }}</td><td><StatusBadge :tone="resultTone(item.result)">{{ resultName(item.result) }}</StatusBadge></td><td class="table-action-column"><RouterLink class="button button-ghost button-sm" :to="adminContextLink('/admin/subscriptions', { user_id: String(item.user_id), subscription: String(item.subscription_id) })">查看订阅</RouterLink></td></tr></tbody>
      </DataTable>
