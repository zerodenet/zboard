<template>
  <section class="stack">
    <header><p class="page-eyebrow">账户安全</p><h1>第三方登录</h1><p>绑定后可使用第三方账号登录。绑定和解绑都需要确认当前 ZBoard 密码。</p></header>
    ...
    <section class="panel stack">
      ...
      <div v-for="identity in identities" :key="identity.id" class="identity-row">
        ...
        <UiButton type="button" variant="secondary" :disabled="busy || !password" @click="unlink(identity.id)">解绑</UiButton>
      </div>
      <h2>可用提供方</h2>
      <p v-if="!providers.length">当前没有可用的第三方登录提供方。</p>
      <div v-for="provider in providers" :key="provider.id" class="identity-row">
        <strong>{{ provider.name }}</strong>
        <UiButton type="button" :disabled="busy || !password || identities.some(item => item.plugin_id === provider.id)" @click="bind(provider.id)">{{ identities.some(item => item.plugin_id === provider.id) ? '已绑定' : '绑定账号' }}</UiButton>
      </div>
    </section>
  </section>
</template>
...
import { bindExternalIdentity, fetchExternalIdentities, fetchIdentityProviders, navigateToIdentityProvider, unlinkExternalIdentity, type ExternalIdentity, type IdentityProvider } from '../../api/identities'
...
async function load() { [providers.value, identities.value] = await Promise.all([fetchIdentityProviders(), fetchExternalIdentities()]) }
