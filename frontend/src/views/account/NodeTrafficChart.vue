<template>
  <section class="panel node-traffic" aria-labelledby="node-traffic-title">
    <header class="panel-header node-traffic-header">
      ...
      <span v-if="data" class="bucket-label">{{ bucketLabel(data.bucket) }}粒度</span>
    </header>

    <p v-if="data?.truncated" class="series-limit-notice">
      为控制查询与浏览器开销，仅展示流量最高的 {{ data.node_limit }} 个节点；选择单个节点可查看更长时间范围。
    </p>
    ...
    <div v-if="showRanking && series.length" class="node-ranking" aria-label="节点流量排行">
      <div class="node-ranking-heading"><strong>节点流量排行</strong><span>按所选日期范围内计费流量从高到低</span></div>
      <ol>
        <li v-for="(item, index) in series" :key="item.nodeId">
          <span class="ranking-index">{{ index + 1 }}</span>
          <span class="ranking-name">{{ item.label }}</span>
          <span class="ranking-track"><i :style="{ width: `${rankingPercent(item.total)}%`, background: item.color }" /></span>
...
function bucketLabel(value: TrafficNodeSeries['bucket']) {
  return value === 'minute' ? '分钟' : value === 'day' ? '天' : '小时'
}
...
function rankingPercent(total: number) {
  return Math.max(2, (total / Math.max(1, series.value[0]?.total || 1)) * 100)
}
...
<style scoped>
.node-traffic { min-width: 0; }.node-traffic-header { align-items: flex-start; }.bucket-label { flex: none; padding: 5px 8px; border: 1px solid var(--line); border-radius: 999px; color: var(--muted); background: var(--surface-soft); font-size: 10px; font-weight: 700; }.series-limit-notice { margin: 0 18px 8px; padding: 8px 10px; border: 1px solid var(--warning); border-radius: 8px; color: var(--muted); background: var(--surface-soft); font-size: 10px; }.chart-state { min-height: 220px; display: flex; align-items: center; justify-content: center; gap: 9px; color: var(--muted); font-size: 12px; }.chart-state.empty { text-align: left; }.chart-state.empty strong { display: block; color: var(--text); }.chart-state p { margin: 3px 0 0; }.node-chart-body { display: grid; gap: 10px; padding: 10px 18px 18px; }.node-legend { display: flex; flex-wrap: wrap; gap: 8px 16px; padding: 0 4px; }.node-legend > span { display: inline-flex; align-items: center; gap: 6px; min-width: 0; color: var(--muted); font-size: 10px; }.node-legend i,.tooltip-row i { width: 14px; height: 3px; flex: none; border-radius: 99px; background: var(--series-color); }.node-legend b { color: var(--text); font-weight: 700; }.chart-stage { position: relative; min-width: 0; overflow: hidden; }.node-chart { width: 100%; min-width: 640px; display: block; outline: none; }.chart-grid line { stroke: var(--line); stroke-width: 1; }.chart-grid text,.axis-labels text { fill: var(--muted); font-size: 9px; }.node-line { fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; vector-effect: non-scaling-stroke; }.hover-layer line { stroke: var(--line-strong, var(--muted)); stroke-width: 1; stroke-dasharray: 4 4; }.chart-tooltip { position: absolute; top: 12px; transform: translateX(-50%); min-width: 190px; max-width: 280px; display: grid; gap: 7px; padding: 10px 11px; border: 1px solid var(--line); border-radius: 9px; background: var(--surface); box-shadow: var(--shadow-md); pointer-events: none; z-index: 3; }.chart-tooltip.align-left { transform: translateX(0); }.chart-tooltip.align-right { transform: translateX(-100%); }.chart-tooltip > strong { font-size: 10px; }.tooltip-row { display: flex; align-items: center; justify-content: space-between; gap: 14px; font-size: 10px; }.tooltip-row span { min-width: 0; display: flex; align-items: center; gap: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--muted); }.tooltip-row b { flex: none; }.node-ranking { display: grid; gap: 10px; padding: 0 18px 18px; }.node-ranking-heading { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; }.node-ranking-heading strong { font-size: 11px; }.node-ranking-heading span { color: var(--muted); font-size: 9px; }.node-ranking ol { display: grid; gap: 6px; margin: 0; padding: 0; list-style: none; }.node-ranking li { display: grid; grid-template-columns: 24px minmax(110px, .5fr) minmax(140px, 1fr) auto; align-items: center; gap: 9px; }.ranking-index { display: grid; width: 22px; height: 22px; place-items: center; border-radius: 6px; background: var(--surface-soft); color: var(--muted); font-size: 9px; font-weight: 700; }.ranking-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 10px; }.ranking-track { height: 7px; overflow: hidden; border-radius: 99px; background: var(--surface-soft); }.ranking-track i { display: block; height: 100%; border-radius: inherit; }.node-ranking li>strong { font-size: 10px; }.node-ranking li:first-child .ranking-index { background: var(--primary-soft); color: var(--primary); }@media (max-width: 720px) { .chart-stage { overflow-x: auto; }.node-chart-body { padding-inline: 12px; }.node-ranking { padding-inline: 12px; }.node-ranking li { grid-template-columns: 24px minmax(100px, 1fr) auto; }.ranking-track { display: none; } }
</style>
