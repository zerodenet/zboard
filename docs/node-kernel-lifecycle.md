...
当前协议部署只通过 SSH 把单个端点 JSON 暂存到 `/etc/zerodenet/protocols/`。Zero 当前加载的是完整运行时配置，因此“文件上传成功”不能称为“协议已生效”。后续部署必须编译节点级完整配置，并通过 Zero 自身的 `validate`、本地控制 socket 和面板心跳共同验收。
...
协议保存仍然只是数据库操作。目标流程中，点击“应用到节点”时，zboard 对该节点的全部启用端点生成一个新的配置修订，而不是逐个文件直接覆盖运行态。新配置先校验，再通过 Zero 本地控制 socket 执行 reload；reload 或健康确认失败则恢复上一 generation。当前版本先由节点页“配置对齐”完成完整 generation 的重启切换。
...
- 第三阶段：配置 generation + validate + reload，把现有协议 SSH 暂存替换为真实生效流程。
- 第四阶段：可控升级/降级、批量调度、灰度和失败节点隔离。

在第三阶段完成之前，协议页面必须继续使用“配置已暂存”等准确文案，不得把单文件上传显示成“协议已生效”。
...
尚未完成的是协议页面保存后的自动 generation/reload、可恢复的远程幂等键、批量/灰度调度，以及 Zero 官方 release 工作流直接发布 musl 制品。现阶段兼容制品由 zboard 运维目录托管；协议变更后需在节点页面主动执行一次“配置对齐”。
