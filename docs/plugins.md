...
权限审查、私有存储、升级迁移与清除策略见 [插件数据与授权](plugin-governance.md)。新包必须先在详情中授权才能打开配置页面或运行。
...
离线导入在弹窗中选择签名 `.zbplugin` 文件，再点击“验证并导入”，不会在选择文件时安装。成功后进入插件详情并保持停用；失败信息留在弹窗中供修正、重试。文件最大 32 MiB；展开最多 64 MiB、512 个内容文件。验证失败不会注册或执行插件。

导入后先检查兼容性、发布者和范围，审查当前包的能力授权，完成待应用的数据迁移，必要时保存配置，再启用。纯页面插件不启动服务进程。服务插件测试使用已保存配置；停用状态下测试临时启动诊断进程，测试完成即退出。

升级先停用，再导入同 ID、同发布者的新版本，最后启用。详情中可切换已保留版本，旧版本必须兼容当前宿主及已提交的数据版本；切换后保持停用，重新确认包授权并验证配置后启用。每插件最多 30 个不同包版本，安装记录最多 200 条，包括卸载墓碑；当前没有自动历史清理接口。

卸载删除程序与页面，保留配置和记录。详情中的“删除保留的配置”仅在卸载后可用，不影响核心用户、订单或凭证。再次安装需重新导入包。配置 JSON 完整替换旧配置，秘密值不回显；请保留安全的原始配置来源。
