...
导入后先检查兼容性、发布者和范围，必要时保存配置，再启用。纯页面插件不启动服务进程。服务插件测试使用已保存配置；停用状态下测试临时启动诊断进程，测试完成即退出。

升级先停用，再导入同 ID、同发布者的新版本，最后启用。详情中可切换已保留版本，旧版本必须兼容当前宿主并能应用当前配置；切换后保持停用。每插件最多 30 个不同包版本，安装记录最多 200 条，包括卸载墓碑；当前没有自动历史清理接口。
