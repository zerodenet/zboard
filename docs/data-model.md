...
- Saving a protocol endpoint queues an immediate node-level publish. The publisher compiles every active endpoint and active subscription credential on that node into one complete Zero runtime configuration, records the desired SHA-256, validates it with the installed binary, atomically switches the generation, restarts Zero, checks the local control socket and waits for a fresh authenticated Connector event.
