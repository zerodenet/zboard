...
mkdir -p "$artifact_dir/rules" "$managed_rule_dir" "$zero_event_spool_dir" "$zero_event_spool_blue_dir" "$zero_event_spool_green_dir" "$database_dir"
chmod 0755 "$artifact_dir" "$artifact_dir/rules"
chmod 0750 "$managed_rule_dir" "$zero_event_spool_dir" "$zero_event_spool_blue_dir" "$zero_event_spool_green_dir" "$database_dir"
...
printf 'Prepared database directory: %s (writable persistent mount)\n' "$database_dir"
