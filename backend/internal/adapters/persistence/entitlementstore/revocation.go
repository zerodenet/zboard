...
func persistCredentialRevocation(tx *gorm.DB, query *gorm.DB, status string, now time.Time) error {
	...
	for _, credential := range credentials {
		...
		if err := networkstore.EnqueueNodePublication(tx, credential.NodeID, credential.ProtocolEndpointID, 0); err != nil {
			return err
		}
	...
	return tx.Model(&model.ProtocolCredential{}).Where("id IN ?", credentialIDs(credentials)).Updates(updates).Error
}
...
func ExpireInTransaction(tx *gorm.DB, userID uint, now time.Time) error {
	...
	credentials := tx.Model(&model.ProtocolCredential{}).
		Where("status IN ? AND subscription_id IN (?)", []string{"active", "prepared"}, inactive)
	return persistCredentialRevocation(tx, credentials, "expired", now)
}

func credentialIDs(credentials []model.ProtocolCredential) []uint {
