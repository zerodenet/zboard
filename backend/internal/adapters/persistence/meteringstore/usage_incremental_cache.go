...
func (c *IncrementalCache) acquire(key [32]byte) *trafficIncrementalEntry {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.entries == nil {
...
func (c *IncrementalCache) Load(base *gorm.DB, bucket UsageBucketSpec, key [32]byte) (metering.UsageStatistics, bool, error) {
	var result metering.UsageStatistics
	entry := c.acquire(key)
	if entry == nil {
	...
	if err != nil {
		...
		if errors.Is(err, errTrafficIncrementalCapacity) {
			...
			return metering.UsageStatistics{}, false, nil
		}
	}
