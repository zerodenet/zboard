...
func loadTrafficReconciliation(db *gorm.DB, userID, subscriptionID uint, now time.Time, paged, issuesOnly bool, offset, limit int) (metering.ReconciliationSnapshot, error) {
	...
	trafficTotalsQuery := func() *gorm.DB {
		return ReconciliationTotalsQuery(db, baseQuery(), userID != 0 || subscriptionID != 0)
	}
	var total int64
	aggregates := metering.ReconciliationAggregates{}
	...
	items := make([]metering.ReconciliationItem, 0, len(subscriptions))
	for _, subscription := range subscriptions {
		...
		items = append(items, metering.ReconciliationItem{
			SubscriptionID: subscription.ID,
			UserID:         subscription.UserID,
			PlanID:         subscription.PlanID,
			Status:         entitlements.EffectiveStatus(entitlements.Subscription(subscription.Subscription), now),
			FlowUsed:       subscription.FlowUsed,
			RecordedBytes:  recorded,
			Difference:     difference,
			Result:         metering.ReconciliationResult(difference),
		})
	}
	return metering.ReconciliationSnapshot{Items: items, Total: total, Aggregates: aggregates}, nil
}
