...
func TestTrafficIncrementalStatisticsTrackAppendMutationAndRollback(t *testing.T) {
	...
	check := func() {
		...
		want, err := LoadUsageStatistics(base, bucket, window)
	...
	if err := f.db.Model(&model.TrafficRecord{}).Where("id = ?", record.ID).Updates(map[string]any{"used_bytes": 39, "node_id": nil, "user_id": nil, "protocol_multiplier_milli": nil, "subscription_id": nil}).Error; err != nil {
		t.Fatal(err)
	}
	check()
	if cache.entries[key].state.version.Revision == initialRevision {
	...
	if err := f.db.Delete(&model.TrafficRecord{}, 1).Error; err != nil {
		t.Fatal(err)
	}
	check()
	...
	if err := f.db.Raw("PRAGMA database_list").Scan(&databases).Error; err != nil {
		t.Fatal(err)
	}
	var path string
	...
	if err := f.db.Exec("PRAGMA recursive_triggers = OFF").Error; err != nil {
		t.Fatal(err)
	}
	// Explicit REPLACE of the highest ID keeps count and high-water unchanged.
	if err := f.db.Exec(`INSERT OR REPLACE INTO traffic_records
...
FROM traffic_records WHERE id=1`).Error; err != nil {
	...
	if err := f.db.Exec("DELETE FROM traffic_ledger_revision").Error; err != nil {
		t.Fatal(err)
	}
	if err := f.db.Exec("UPDATE traffic_records SET used_bytes = 91 WHERE id = (SELECT MAX(id) FROM traffic_records)").Error; err != nil {
		t.Fatal(err)
	}
	if _, used, err := cache.Load(base, bucket, key); err != nil || used {
		t.Fatalf("missing revision used=%v error=%v", used, err)
	}
	if err := f.db.Exec("INSERT INTO traffic_ledger_revision(id,revision) VALUES(1,0)").Error; err != nil {
		t.Fatal(err)
	}
	check()
	...
	if err := f.db.Exec("DROP TRIGGER traffic_ledger_revision_update").Error; err != nil {
		t.Fatal(err)
	}
	if _, used, err := cache.Load(base, bucket, key); err != nil || used {
...
func TestTrafficIncrementalScopesCapacityAndCancellation(t *testing.T) {
	...
	start := time.Date(2026, 9, 1, 0, 0, 0, 0, time.UTC)
	window := UsageWindow{From: start.Add(time.Hour + 5*time.Minute), To: start.Add(time.Hour + 55*time.Minute)}
	cache := &IncrementalCache{}
	for _, name := range []string{"minute", "hour", "day"} {
		bucket, _ := ParseUsageBucket(name)
		bucket = bucket.ForDB(f.db)
		for _, filter := range []string{"1=1", "user_id=1", "protocol_endpoint_id=1", "subscription_id=999", "node_id=1"} {
			base := applyUsageWindow(f.db.Model(&model.TrafficRecord{}).Where(filter), "record_at", window)
			key := UsageSnapshotQueryKey(base, bucket.Name)
			got, used, err := cache.Load(base, bucket, key)
	...
	bucket, _ := ParseUsageBucket("hour")
	bucket = bucket.ForDB(f.db)
	base := applyUsageWindow(f.db.Model(&model.TrafficRecord{}), "record_at", window)
	key := UsageSnapshotQueryKey(base, bucket.Name)
	limited := &IncrementalCache{keyLimit: 1}
	for range 2 {
