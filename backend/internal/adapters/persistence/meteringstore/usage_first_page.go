...
const UsageFirstPageProbeRows = 1024
...
func (b UsageBucketSpec) FirstPageSource(base *gorm.DB, window UsageWindow, limit int) *gorm.DB {
	recent := applyUsageWindow(base.Session(&gorm.Session{}), "record_at", window).
		Select("record_at, user_id, subscription_id, node_id, protocol_multiplier_milli").
		Order("record_at desc").Limit(UsageFirstPageProbeRows)
	...
	if datastore.IsSQLite(base) {
		...
		return applyUsageWindow(base.Session(&gorm.Session{}), "record_at", window).
