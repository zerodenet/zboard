...
type pageActionHostServices struct{}

func (pageActionHostServices) CallPluginHost(context.Context, string, string, string, json.RawMessage) (json.RawMessage, *pluginv1.HostCallError) {
	return nil, &pluginv1.HostCallError{Code: "forbidden"}
}

func (pageActionHostServices) PluginPrincipal(_ context.Context, pluginID string, userID uint) (string, error) {
	return pluginID + ":" + strconv.FormatUint(uint64(userID), 10), nil
}
...
func TestPageActionUsesExactAuthenticatedSessionContext(t *testing.T) {
	...
	manager.SetHostServices(pageActionHostServices{})
	...
	if response["actor_id"] != "example.page:41" || response["page_id"] != "devices" || response["surface"] != "account" || response["action"] != "devices.list" {
