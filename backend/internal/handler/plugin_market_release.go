...
func (h *handlers) AdminPluginMarketDetailHandler(w http.ResponseWriter, r *http.Request) {
	...
	detail, err := h.pluginManager.MarketDetail(r.Context(), pathvar.Vars(r)["id"])
	if err != nil {
		pluginError(w, err)
		return
	}
...
func (h *handlers) AdminPluginMarketInspectHandler(w http.ResponseWriter, r *http.Request) {
	...
	preview, err := h.pluginManager.PreviewMarket(r.Context(), pathvar.Vars(r)["id"])
	if err != nil {
		pluginError(w, err)
		return
	}
...
func (h *handlers) AdminPluginMarketInstallHandler(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.pluginManager.InstallMarketConfirmed(r.Context(), pathvar.Vars(r)["id"], body.Digest, body.Fingerprint, claims.Email)
	if err != nil {
		pluginError(w, err)
		return
	}
