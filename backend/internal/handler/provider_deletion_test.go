...
func TestProviderDeletionHTTPRetainsUnknownExecutionEvidence(t *testing.T) {
	...
	f.h.ProviderAccountDeleteHandler(w, announcementRequest(http.MethodDelete, fmt.Sprintf("/api/v1/admin/provider-accounts/%d", account.ID), f.admin, ""))
