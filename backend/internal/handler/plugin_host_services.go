...
func (h *handlers) CallPluginHost(ctx context.Context, pluginID, capability, operation string, payload json.RawMessage) (json.RawMessage, *pluginv1.HostCallError) {
	...
	case plugins.AccountAssertionCapability:
		...
		result, err = h.pluginAssertAccount(ctx, payload)
	case plugins.SubscriptionProjectionCapability:
		switch operation {
		case "subscriptions.list":
			result, err = h.pluginListSubscriptions(ctx, payload)
		case "subscriptions.get":
			result, err = h.pluginGetSubscription(ctx, payload)
		default:
			return nil, hostCallError("forbidden")
		}
	case plugins.MessageProjectionCapability:
		switch operation {
		case "messages.list":
			result, err = h.pluginListMessages(ctx, payload)
		case "messages.get":
			result, err = h.pluginGetMessage(ctx, payload)
		case "messages.mark-read":
			result, err = h.pluginMarkMessageRead(ctx, payload)
		default:
			return nil, hostCallError("forbidden")
		}
	default:
...
func (h *handlers) pluginAssertAccount(ctx context.Context, payload json.RawMessage) (any, *pluginv1.HostCallError) {
	...
	return pluginv1.Principal{ID: strconv.FormatUint(uint64(user.ID), 10), Display: display, Admin: user.IsAdmin}, nil
}
...
func (h *handlers) pluginListSubscriptions(ctx context.Context, payload json.RawMessage) (any, *pluginv1.HostCallError) {
	...
	userID, callErr := h.pluginPrincipal(ctx, request.PrincipalID)
	if callErr != nil {
		return nil, callErr
	}
...
func (h *handlers) pluginGetMessage(ctx context.Context, payload json.RawMessage) (any, *pluginv1.HostCallError) {
	...
	userID, messageID, user, callErr := h.pluginMessageTarget(ctx, request.PrincipalID, request.MessageID)
	if callErr != nil {
		return nil, callErr
	}
...
func (h *handlers) pluginPrincipal(ctx context.Context, value string) (uint, *pluginv1.HostCallError) {
	parsed, err := strconv.ParseUint(value, 10, 64)
	if err != nil || parsed == 0 {
		return 0, hostCallError("invalid_request")
	}
...
func (h *handlers) pluginMessageTarget(ctx context.Context, principalID, rawMessageID string) (uint, uint, model.User, *pluginv1.HostCallError) {
	userID, callErr := h.pluginPrincipal(ctx, principalID)
	if callErr != nil {
